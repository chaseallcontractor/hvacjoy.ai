...
// NEW: robust extractor for the last question, even without a "?"
function extractLastQuestionLine(text = '') {
  if (!text) return null;
  const trimmed = String(text).trim();

  // Try the usual: last sentence ending with "?"
  const m = trimmed.match(/([^?]*\?)[^?]*$/);
  if (m) return m[1].trim();

  // Fallback: detect common yes/no prompts without a question mark
  const patterns = [
  ...
  for (const re of patterns) {
    ...
    if (mm) return trimmed; // treat whole line as the question
  }
  return null;
}
...
function parsePhone(text) {
  ...
  if (digits.length === 10) return { full: fmt(digits), complete: true };
...
function isAffirmation(text='') {
  ...
  return /\b(yes|yep|yeah|yah|ya|yup|sure|ok|okay|correct|roger|affirmative|uh huh|uh-huh|that'?s (right|correct)|looks good|sounds good|please continue|go ahead|proceed|continue|that works)\b/i.test(norm);
}
function isNegation(text='') {
  // removed \bknow\b false-positive
...
// -------- NEW: Name capture (avoid LLM roundtrip for a simple slot) --------
function handleNameProgress(speech, slots, lastQ) {
  const s = { ...(slots || {}) };
  const askedForName = /full name|your name/i.test(lastQ || '') || (!s.full_name && /^what('| i)s your (full )?name\??/i.test(lastQ || ''));
  if (!askedForName || s.full_name) {
    return { slots: s, reply: null, handled: false };
  }
  const raw = String(speech || '').trim();
  // Simple “looks like a name” check: 2–5 alphabetic tokens (allow a dot)
  const tokens = raw.split(/\s+/).filter(w => /[A-Za-z][A-Za-z.'-]*/.test(w));
  if (tokens.length >= 1) {
    // If 2+ tokens, use first + last; else keep single token
    const first = tokens[0].replace(/[^\w.'-]+/g,'');
    const last  = tokens.length > 1 ? tokens[tokens.length - 1].replace(/[^\w.'-]+/g,'') : '';
    s.full_name = last ? `${first} ${last}` : first;
    const prompt = nextMissingPrompt(s) || 'Great—thanks. What day works for your visit? The earliest is tomorrow.';
    return { slots: s, reply: prompt, handled: true };
  }
  return { slots: s, reply: null, handled: false };
}
...
function handleAddressProgress_STRICT(speech, slots) {
  ...
  if (full) {
    ...
    return { slots: s, reply: `Thank you. I have ${a.line1}, ${a.city}, ${a.state} ${a.zip}. Is that correct?`, handled: true };
  }
  ...
  if (line1 && cityZip) {
    s.service_address = { ...(s.service_address || {}), line1, city: cityZip.city, zip: cityZip.zip, state: cityZip.state || DEFAULT_STATE };
    s.address_confirm_pending = true;
    const a = s.service_address;
    return { slots: s, reply: `Thank you. I have ${a.line1}, ${a.city}, ${a.state} ${a.zip}. Is that correct?`, handled: true };
  }
  return { slots: s, reply: 'Please say the full service address—street, city, and zip.', handled: true };
}
...
function handlePhoneProgress(speech, slots) {
  ...
  if (!heardDigits && !existing) return { slots: s, reply: null, handled: false };

  const combined = (existing + heardDigits).slice(0, 16);
  ...
  if (parsed && parsed.complete) {
    ...
    s._phone_partial = '';
    return { slots: s, reply: `Thanks. I have your callback number as ${parsed.full}. Is that correct?`, handled: true };
  }

  if (combined.length > 0) {
    s._phone_partial = combined;
    const remaining = Math.max(10 - combined.length, 1);
    return { slots: s, reply: `I heard the beginning. Please say the remaining ${remaining} digit${remaining > 1 ? 's' : ''} of the 10-digit number.`, handled: true };
  }
  return { slots: s, reply: null, handled: false };
}
  ...
  try {
    ...
    if (/\b(smoke|sparks?|gas (smell|leak)|carbon monoxide|fire|danger|burning smell)\b/i.test(speech || '')) {
      return res.status(200).json({
        ...
        slots: mergedSlots, done: false, goodbye: null, needs_confirmation: true, model: 'gpt-4o-mini', usage: null,
      });
    }
    ...
    if (mergedSlots.confirmation_pending === true) {
      if (isAffirmation(speech) && !isNegation(speech)) {
        ...
        return res.status(200).json({ reply: E(""), slots: mergedSlots, done: true, goodbye: bye, needs_confirmation: false, model: 'gpt-4o-mini', usage: null });
      }
      if (isNegation(speech) || /\b(change|update|fix|wrong|not (right|correct))\b/i.test(speech)) {
        mergedSlots.confirmation_pending = false;
        return res.status(200).json({ reply: E("No problem—what would you like to change?"), slots: mergedSlots, done: false, goodbye: null, needs_confirmation: false, model: 'gpt-4o-mini', usage: null });
      }
      return res.status(200).json({ reply: E("If everything is correct, please say “yes.” Otherwise, tell me what to change."), slots: mergedSlots, done: false, goodbye: null, needs_confirmation: true, model: 'gpt-4o-mini', usage: null });
    }
    ...
    if (mergedSlots.address_confirm_pending === true) {
      ...
      if (isAffirmation(speech) && !isNegation(speech)) {
        ...
        return res.status(200).json({ reply: E(prompt), slots: mergedSlots, done: false, goodbye: null, needs_confirmation: false, model: 'gpt-4o-mini', usage: null });
      }
      if (isNegation(speech)) {
        mergedSlots.address_confirm_pending = false;
        mergedSlots.service_address = { line1: null, line2: null, city: null, state: null, zip: null };
        return res.status(200).json({ reply: E('Sorry about that—what is the correct full address? Please say it in one sentence: street, city, and zip.'), slots: mergedSlots, done: false, goodbye: null, needs_confirmation: false, model: 'gpt-4o-mini', usage: null });
      }
      return res.status(200).json({ reply: E(`Is this address correct: ${[addr.line1, addr.city, (addr.state || DEFAULT_STATE), addr.zip].filter(Boolean).join(', ')}? Please say yes or no.`), slots: mergedSlots, done: false, goodbye: null, needs_confirmation: false, model: 'gpt-4o-mini', usage: null });
    }
    ...
    if (mergedSlots.callback_confirm_pending === true) {
      ...
      if (!n) {
        ...
        return res.status(200).json({ reply: E('What’s the full 10-digit callback number?'), slots: mergedSlots, done: false, goodbye: null, needs_confirmation: false, model: 'gpt-4o-mini', usage: null });
      }
      ...
      if (isNegation(speech)) {
        ...
        return res.status(200).json({ reply: E('No problem—what’s the full 10-digit callback number?'), slots: mergedSlots, done: false, goodbye: null, needs_confirmation: false, model: 'gpt-4o-mini', usage: null });
      }
      return res.status(200).json({ reply: E(`I have ${n} as your callback number. Is that correct? Please say yes or no.`), slots: mergedSlots, done: false, goodbye: null, needs_confirmation: false, model: 'gpt-4o-mini', usage: null });
    }

    // ---- Strict address capture
    const addrProgress = handleAddressProgress_STRICT(speech, mergedSlots);
    if (addrProgress.handled) {
      mergedSlots = addrProgress.slots;
      return res.status(200).json({ reply: E(addrProgress.reply), slots: mergedSlots, done: false, goodbye: null, needs_confirmation: false, model: 'gpt-4o-mini', usage: null });
    }
    ...
    if (phoneProgress.handled) {
      ...
      return res.status(200).json({ reply: E(phoneProgress.reply), slots: mergedSlots, done: false, goodbye: null, needs_confirmation: false, model: 'gpt-4o-mini', usage: null });
    }
    ...
    // ---- Read last question now (used by next two fast-paths)
    const priorLastQuestion = getLastAssistantQuestion(history) || '';
    const lastQ = lastQuestion || priorLastQuestion;

    // ---- NEW: name progress (when we just asked for it)
    const nameProgress = handleNameProgress(speech, mergedSlots, lastQ);
    if (nameProgress.handled) {
      mergedSlots = nameProgress.slots;
      return res.status(200).json({ reply: E(nameProgress.reply), slots: mergedSlots, done: false, goodbye: null, needs_confirmation: false, model: 'gpt-4o-mini', usage: null });
    }
    ...
    // NEW: short "no" to a yes/no → re-ask same question
    if (!mergedSlots.confirmation_pending && !mergedSlots.address_confirm_pending && !mergedSlots.callback_confirm_pending) {
      if (isNegation(speech) && isYesNoQuestion(lastQ)) {
        return res.status(200).json({ reply: E(`No problem—let’s try that again. ${lastQ}`), slots: mergedSlots, done: false, goodbye: null, needs_confirmation: false, model: 'gpt-4o-mini', usage: null });
      }
    ...
    // ===== Plain “yes” to an open-ended prompt → advance =====
    if (!mergedSlots.confirmation_pending && !mergedSlots.address_confirm_pending && !mergedSlots.callback_confirm_pending && isAffirmation(speech) && !isYesNoQuestion(lastQ)) {
      const prompt = nextMissingPrompt(mergedSlots) || 'Great—thanks. What day works for your visit? The earliest is tomorrow.';
      return res.status(200).json({ reply: E(prompt), slots: mergedSlots, done: false, goodbye: null, needs_confirmation: false, model: 'gpt-4o-mini', usage: null });
    }
    // ====================================================================
    ...
    if (!resp?.ok) {
      ...
      return res.status(200).json({
        ...
        slots: mergedSlots, done: false, goodbye: null, needs_confirmation: false, model: 'gpt-4o-mini', usage: null,
      });
    }
    ...
    try { parsed = JSON.parse(raw); }
      ...
      parsed = { reply: 'Thanks. What’s the next detail—your full service address: street, city, and zip?', slots: {}, done: false, goodbye: null, needs_confirmation: false };
    }
    ...
    if (serverDone) {
      try {
        ...
        const description = [
          ...
          (s.thermostat && s.thermostat.setpoint != null && s.thermostat.current != null) ? `Thermostat: set ${s.thermostat.setpoint}, current ${s.thermostat.current}` : null,
        ...
        let startEnd;
        if (safeTimeHHMM) startEnd = timeToTimes(safeDateISO, safeTimeHHMM, DEFAULT_TZ);
        else startEnd = windowToTimes(safeDateISO, sNorm.preferred_window, DEFAULT_TZ);
      ...
      const callAheadBit = (sNorm.call_ahead === false) ? ' We will arrive within your window without a call-ahead.' : ' We’ll call ahead before arriving.';
      ...
      return res.status(200).json({ reply: E(""), slots: mergedSlots, done: true, goodbye: goodbyeLine, needs_confirmation: false, model: 'gpt-4o-mini', usage: null });
    }

    return res.status(200).json({ reply: parsed.reply, slots: mergedSlots, done: false, goodbye: null, needs_confirmation: false, model: 'gpt-4o-mini', usage: null });
  } catch (err) {
    console.error('chat handler error', err);
    return res.status(200).json({
      reply: enforceHVACBranding("I caught that. When you’re ready, please say the full service address—street, city, and zip."),
      slots: (req.body && req.body.lastSlots) || {}, done: false, goodbye: null, needs_confirmation: false,
